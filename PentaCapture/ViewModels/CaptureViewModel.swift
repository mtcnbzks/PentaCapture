...
class CaptureViewModel: ObservableObject {
    ...
    func startCapture() {
        ...
        if faceTrackingService.isSupported {
            ...
            print("❌ ARKit Face Tracking not available - app requires TrueDepth camera (iPhone X or later)")
    ...
    private func performValidation(_ pixelBuffer: CVPixelBuffer?) async {
        ...
        // Log pose data
        if validationLogCount % 30 == 1 {
            print("   ✅ Pose: Y=\(String(format: "%.1f", headPose.yawDegrees))° P=\(String(format: "%.1f", headPose.pitchDegrees))° R=\(String(format: "%.1f", headPose.rollDegrees))°")
        }
        ...
        await MainActor.run {
            ...
            if validationLogCount % 30 == 1 {
                ...
                print("         Pitch: \(String(format: "%.1f", currentPitch))° (target: \(String(format: "%.1f", currentAngle.targetPitch))°, tolerance: ±\(String(format: "%.1f", currentAngle.pitchTolerance))°)")
                print("         Pitch Error: \(String(format: "%.1f", abs(pitchError)))°")
                if let currentYaw = currentYaw, let targetYaw = currentAngle.targetYaw, let yawError = yawError {
                    print("         Yaw: \(String(format: "%.1f", currentYaw))° (target: \(String(format: "%.1f", targetYaw))°, tolerance: ±\(String(format: "%.1f", currentAngle.yawTolerance))°)")
                    print("         Yaw Error: \(String(format: "%.1f", abs(yawError)))°")
                }
                print("      Detection: \(detectionValidation.status)")
                print("      Stable: \(isStable) (\(String(format: "%.2f", stabilityDuration))s / \(String(format: "%.1f", PoseValidation.requiredStabilityDuration))s)")
    ...
    private func triggerAutoCapture() {
        ...
        if let pose = lockedPose {
            print("🔒 Locked pose: Yaw=\(String(format: "%.1f", pose.yawDegrees))° Pitch=\(String(format: "%.1f", pose.pitchDegrees))°")
        }
            ...
            for count in (1...3).reversed() {
                ...
                for _ in 0..<10 {
                    ...
                    if let locked = self.lockedPose, let current = self.faceTrackingService.currentHeadPose {
                        ...
                        if yawDiff > self.countdownMovementTolerance || pitchDiff > self.countdownMovementTolerance {
                            print("⚠️ Excessive movement during countdown! Yaw: \(String(format: "%.1f", yawDiff))° Pitch: \(String(format: "%.1f", pitchDiff))°")
                            self.isCountingDown = false
                            self.lockedPose = nil
                            self.audioService.playFeedback(.error)
                            return
    ...
    func manualCapture() async {
        ...
        if currentValidation?.detectionValidation.isDetected == false {
            ...
            errorMessage = "Yüz tespit edilemedi. Lütfen yüzünüzün kamera görüş alanında olduğundan emin olun."
            audioService.playFeedback(.error)
            return
        }
    ...
    func retakeAngle(_ angle: CaptureAngle) {
        ...
        let needsRestart = faceTrackingService.isSupported ?
...
extension CaptureViewModel {
    ...
    static var previewWithPhotos: CaptureViewModel {
        let vm = CaptureViewModel()
        // Add mock photos for preview
        if let image = UIImage(systemName: "person.circle.fill") {
            for angle in CaptureAngle.allCases.prefix(3) {
                let photo = CapturedPhoto(angle: angle, image: image)
                vm.session.addPhoto(photo)
            }
        }
        return vm
    }
